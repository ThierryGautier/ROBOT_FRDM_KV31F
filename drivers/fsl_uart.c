...
#ifdef UART_TIME_LOG
uint32_t gu32OldTimer = 0;
uint32_t gau32ArrayOfDeltaTimerInus[256];
uint32_t gu32Index = 0;
bool gbFirstCall = true;
#endif
...
static void UART_WriteNonBlocking(UART_Type *base, const uint8_t *data, size_t length)
{
    ...
    {
#ifdef UART_TIME_LOG
		uint32_t lu32PIT0Value = PIT_GetCurrentTimerCount(PIT, kPIT_Chnl_0);
    	if(gbFirstCall == true)
    	{
    		gu32OldTimer = lu32PIT0Value;
    		gbFirstCall = false;
    		gau32ArrayOfDeltaTimerInus[gu32Index++] = 0;
    	}
    	else
    	{
			gau32ArrayOfDeltaTimerInus[gu32Index++] = (gu32OldTimer-lu32PIT0Value)/60;
			if(gu32Index > 256) gu32Index = 0;
			gu32OldTimer = lu32PIT0Value;
    	}
#endif
		base->D = data[i];
...
#ifdef UART_TIME_LOG
            gau32ArrayOfDeltaTimerInus[gu32Index++] = 0xDEADCAFE;
            if(gu32Index > 256) gu32Index = 0;
#endif
        }
    }
